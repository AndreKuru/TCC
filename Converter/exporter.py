...
def export_configurations(
    ...
    lines.append("    constant NODES_TO_WRITE             : natural := " + str(nodes_to_write) + ";")
...
def initialize_memory_file() -> list[str]:
    ...
    lines.append("        nodes_to_write      : natural;")
    lines.append("        nodes_in_parallel   : natural")
    lines.append("    );")
    lines.append("    port(")
    lines.append("        clk, write_in           : in  std_logic; ")
    lines.append("        node_data_write         : in  std_logic_vector(nodes_to_write * node_size - 1 downto 0); ")
    lines.append("        base_address_to_write    : in  std_logic_vector(node_address_size - 1 downto 0);")
...
def finalize_memory_file(lines: list[str]) -> list[str]:
    ...
    lines.append("            Data_serialize : for i in 0 to nodes_to_write - 1 loop")
    lines.append("                ram_data(to_integer((i * node_address_size) + unsigned(base_address_to_write(node_address_size - 1 downto 0)))) ")
    lines.append("                        <= ")
    lines.append("                        node_data_write(node_size * (i + 1) - 1 downto node_size * i);")
    ...
    lines.append("")
    lines.append("    begin")
    lines.append("        node_data_read(node_data_end downto node_data_start) <= ram_data(to_integer(unsigned(node_addresses_read(node_address_end downto node_address_start))));")
...
def initialize_testbench() -> list[str]:
    ...
    lines.append("        nodes_to_write              : natural := NODES_TO_WRITE")
    lines.append("    );")
    lines.append("end accelerator_tb;")
    lines.append("")
    lines.append("architecture tb of accelerator_tb is")
    lines.append("")
    lines.append("signal clk, reset, write_in   : std_logic;")
    lines.append("signal features               : std_logic_vector(threshold_size * features_amount - 1 downto 0);")
    lines.append("signal data_to_write          : std_logic_vector(node_size * nodes_to_write - 1 downto 0);")
    lines.append("signal base_address_to_write  : std_logic_vector(levels_in_memory - 1 downto 0);")
    lines.append("signal ready                  : std_logic;")
    lines.append("signal class                  : std_logic_vector(class_size - 1 downto 0);")
    lines.append("")
    lines.append("constant clkp : time := 17.37 ns;")
    lines.append("")
    lines.append("begin")
    lines.append("    write_in <= '0';")
    lines.append("    UUT: entity work.accelerator ")
    lines.append("        port map(")
    lines.append("            clk                 => clk,")
    lines.append("            reset               => reset,")
    lines.append("            write_in            => write_in,")
    lines.append("            features            => features,")
    lines.append("            data_to_write       => data_to_write,")
    lines.append("            base_address_to_write  => base_address_to_write,")
    ...
    lines.append("        data_to_write          <= (others => '0');")
    lines.append("        base_address_to_write  <= (others => '0');")
