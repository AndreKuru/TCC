...
def sort_input_tree(tree: Input_tree) -> tuple[dict, int]:
    ...
    while len(queue) > 0:
        ...
        if input_node in leaf_input_indexes:
            ...
            if child_left != -1:
                max_input_index = sort_children(child_left, queue, queued, input_node, input_indexes, new_indexes, 
                    is_right_child=0)

            if child_right != -1:
                max_input_index = sort_children(child_right, queue, queued, input_node, input_indexes, new_indexes, 
                    is_right_child=1)

    return input_indexes, max_input_index, new_indexes

def set_leaf_children(new_index: int, limit_of_new_indexes: int, child_leaf_values: dict, value: int) -> None:
    ...
    for new_index in range(limit_of_new_indexes):
        ...
        if new_index in input_indexes:
            ...
            if tree.children_left[input_index] == -1 and tree.children_right[input_index] == -1:
                ...
                set_leaf_children(new_index, limit_of_new_indexes, child_leaf_values, value)
